...
AGENT_CONFIGS = [
    ...
    AgentConfig("Agent_1", "A1_V_REVERSAL_AGGRESSIVE", "V_REVERSAL", p_entry_threshold=0.60, p_hold_threshold=0.30, overheat_z_bias=2.8, position_scale=0.6),
    AgentConfig("Agent_2", "A2_V_REVERSAL_MODERATE", "V_REVERSAL", p_entry_threshold=0.75, p_hold_threshold=0.30, overheat_z_bias=2.8, position_scale=0.8),
    AgentConfig("Agent_3", "A3_V_REVERSAL_CONSERVATIVE", "V_REVERSAL", p_entry_threshold=0.85, p_hold_threshold=0.30, overheat_z_bias=2.8, position_scale=1.0),
    
    # 2. 波段續攻組 (Agent 4~6)：overheat_z_bias=2.3, p_hold=0.40
    AgentConfig("Agent_4", "A4_MOMENTUM_AGGRESSIVE", "MOMENTUM", p_entry_threshold=0.60, p_hold_threshold=0.40, overheat_z_bias=2.3, position_scale=0.6),
    AgentConfig("Agent_5", "A5_MOMENTUM_MODERATE", "MOMENTUM", p_entry_threshold=0.75, p_hold_threshold=0.40, overheat_z_bias=2.3, position_scale=0.8),
    AgentConfig("Agent_6", "A6_MOMENTUM_CONSERVATIVE", "MOMENTUM", p_entry_threshold=0.85, p_hold_threshold=0.40, overheat_z_bias=2.3, position_scale=1.0),
    
    # 3. 高檔防禦組 (Agent 7~9)：overheat_z_bias=1.9~2.1, p_hold=0.55
    AgentConfig("Agent_7", "A7_TOP_DEFENSE_AGGRESSIVE", "TOP_DEFENSE", p_entry_threshold=0.60, p_hold_threshold=0.55, overheat_z_bias=2.1, position_scale=0.6),
    AgentConfig("Agent_8", "A8_TOP_DEFENSE_MODERATE", "TOP_DEFENSE", p_entry_threshold=0.75, p_hold_threshold=0.55, overheat_z_bias=2.0, position_scale=0.8),
    AgentConfig("Agent_9", "A9_TOP_DEFENSE_CONSERVATIVE", "TOP_DEFENSE", p_entry_threshold=0.85, p_hold_threshold=0.55, overheat_z_bias=1.9, position_scale=1.0),
]
...
def run_agent_backtest(df_backtest: pd.DataFrame, agent_cfg: AgentConfig, initial_capital: float = 100_000) -> dict:
    ...
    for idx, row in df_backtest.iterrows():
        ...
        # 實施均線支撐自適應自適應勝率遲滯防線：
        # 當股價在 MA20 上方（z_bias >= -0.80，多頭格局未破壞）時，允許勝率跌破 0.50 依然續抱以吃足大波段；
        # 當股價已跌破均線（z_bias < -0.80，趨勢已弱）時，勝率跌破 0.50 則當天立刻一擊平倉，拒絕 any 空頭磨損。
        # 進場與高檔過熱逃頂（z_bias >= overheat_z_bias）維持最敏銳的原始真實值。
        p_conj_for_eval = p_conj
        if current_position > 0.0 and z_bias < overheat_z_bias:
            if z_bias >= -0.80:
                if p_conj < 0.50:
                    p_conj_for_eval = 0.51
                
        # 進場信心自適應提拔補償：
        # 僅在安全的中低檔 (z_bias <= 0.50) 下：
        # 若預測勝率中等以上 (>=0.65)，且主力籌碼大買 (has_major_chip) 或者是極度超賣 (z_bias <= -1.2)，
        # 說明此時是極佳的安全起漲底部。我們將勝率提拔至 0.76，滿倉出擊，精準抄底。高檔時則嚴格執行 0.75 門檻防止追高。
        if current_position == 0.0:
            has_major_chip = float(row.get('Big_Player_Force', 0.0)) > 0.0 or float(row.get('Foreign_Net_Buy', 0.0)) > 0.0 or float(row.get('foreign_net_buy', 0.0)) > 0.0 or float(row.get('major_chip_ratio', 0.0)) > 0.0
            is_oversold = z_bias <= -1.2
            if z_bias <= 0.50 and p_conj >= 0.65:
                if has_major_chip or is_oversold:
                    p_conj_for_eval = 0.76
